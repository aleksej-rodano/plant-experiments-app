...
insert into public.tips (title, content)
...
- 1–2 nodes per cutting is plenty; more just wastes the mother plant.$tip$),
...
- Thin stems (pothos, philodendron): propagate straight away.$tip$),
...
Rule of thumb: move to soil once roots are 3–5 cm.$tip$),
...
- Too much can burn the stem and stall rooting.$tip$),
...
- Once roots are a few cm long, open it wider each day for a week, then remove the cover.$tip$),
...
- A seedling heat mat under the tray makes a big difference.$tip$),
...
- Algae on the glass is harmless; wipe it at changes.$tip$),
...
- Restart in fresh water or damp sphagnum, out of direct sun.$tip$),
...
- Top growth often pauses while roots adjust. That's normal.$tip$),
...
- Too cold — rooting stops below about 18 °C.$tip$),
...
- Don't fertilise water-prop cuttings — plain water is enough.$tip$),
...
- Repeat a promising result before trusting it.$tip$),
...
Winter — slowest. Expect roughly double the time, or wait for spring.$tip$)
) as v(title, content)
where not exists (
