...
import { CARE_TILE_CLASS, careStatus, type CareSchedule } from '../lib/utils/care'

interface Props {
  /** The folder or experiment whose recurring chore this shows. */
  schedule: CareSchedule
  /** Route to the form that edits this schedule. */
  editHref: string
  /** Router state handed to that form so it can seed without a refetch. */
  editState?: unknown
...
/**
 * The recurring-chore status for a folder or experiment — what's due, when, and
 * a one-tap way to reset the clock. Falls back to a quiet prompt when nothing is
 * scheduled yet.
 */
export default function CareBanner({
  ...
  const status = careStatus(schedule)
  ...
  if (!status) {
    return (
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-surface-container px-3 py-2 text-sm text-on-surface-variant">
        ...
        <Link to={editHref} state={editState} className="font-medium underline">
          Set one up
        </Link>
      </div>
    )
      ...
      <span className="flex min-w-0 items-center gap-2">
        ...
        <span className="min-w-0">
          ...
          <span className="block text-xs opacity-80">
            ...
            {schedule.care_last_done_on
              ? ` · last done ${schedule.care_last_done_on}`
              : ' · never marked done'}
          </span>
        </span>
      </span>
